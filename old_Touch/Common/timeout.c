...
/** @defgroup STM32_EVAL_Private_Functions
  ...
  */
	...
	{
		uTO->Enable=true;
		uTO->Counter=TO_SysTickCounter;
    uTO->Time=setTime;
    uTO->CntStart=TO_SysTickCounter;
		uTO->CntTaget=setTime+uTO->Counter;
    if(setTime<(UINT32_MAX-uTO->Counter))uTO->Over=false;
    else uTO->Over=true;
	}
  ...
  */
    ...
    if(uTO->Enable)
      ...
      {
        if(uTO->Counter<uTO->CntStart)uTO->Over=0;         
      }
		}
	}
