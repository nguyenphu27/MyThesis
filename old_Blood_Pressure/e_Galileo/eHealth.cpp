...
//***************************************************************
	...
	void eHealthClass::readGlucometer(void)
	{
		...
		Serial1.write(0x55); // Start communication command. 
		delay(1000); // Wait while receiving data.

		Serial1.print("\n"); 
		if (Serial1.available() > 0) {
			length = Serial1.read();// The protocol sends the number of measures 
			Serial1.read(); // Read one dummy data

			for (int i = 0; i<length; i++) { // The protocol sends data in this order 
				glucoseDataVector[i].year = Serial1.read(); 
				glucoseDataVector[i].month = Serial1.read();
				glucoseDataVector[i].day = Serial1.read();
				glucoseDataVector[i].hour = Serial1.read();
				glucoseDataVector[i].minutes = Serial1.read();

				Serial1.read(); // Byte of separation must be 0x00.

				glucoseDataVector[i].glucose = Serial1.read();
				glucoseDataVector[i].meridian = Serial1.read();

				Serial1.read(); // CheckSum 1
				Serial1.read(); // CheckSum 2			
			}
		}
		digitalWrite(5, LOW);	
	}
